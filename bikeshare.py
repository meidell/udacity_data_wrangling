...
def get_time_period():
    ...
    while valid_input == False:
      if (time_period == "day" or time_period == "month" or time_period == "none"):
        valid_input = True
      else:
        valid_input = False
...
def popular_day(data, time_period):
    ...
    for line in data:
        ...
        my_date = datetime.date(int(text[:2]), int(text[3:5]), int(text[6:7]))
        if(monthly == True and text[3:5] == MONTHS_NUM[time_period]):
...
def statistics():
    ...
    print("The youngest user was born in: ",li
